...
export async function StorefrontHeader() {
  ...
  return <header className="sticky top-0 z-40 border-b bg-white/95 backdrop-blur">
    <div className="mx-auto flex max-w-7xl items-center gap-4 px-4 py-4">
      ...
      <nav className="ml-auto flex items-center gap-4 text-sm"><Link href="/wishlist">Wishlist</Link><Link href="/cart">Cart ({cartCount})</Link><Link href={session?.user ? "/account" : "/login"}>{session?.user ? "Account" : "Sign in"}</Link></nav>
    </div>
